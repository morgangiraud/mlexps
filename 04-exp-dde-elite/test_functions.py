...
from functions import (create_iso_mut, create_line_mut, behaviour_func, fitness_func)
...
class TestIsoMut(unittest.TestCase):
    def test_iso_mut(self):
        ...
        np.testing.assert_almost_equal(
            output, [[0.9472, 0.8927], [1.0865, -0.2302], [0.1745, -0.0761]], decimal=4
        )

        N = 10
        _, output = create_iso_mut(sigma)(N, archive)
        np.testing.assert_almost_equal(
            output, [[0.0197, 0.8455], [0.9912, 0.0852], [0.0677, -0.0107], [1.0725, 1.0935]],
            decimal=4
        )
...
class TestLineMut(unittest.TestCase):
    def test_line_mut(self):
        ...
        np.testing.assert_almost_equal(
            output, [[0.8398, 0.8522], [1.0483, 0.0188], [-0.0465, 0.0862]], decimal=4
        )
        ...
        _, output = create_line_mut(sigma1, sigma2)(N, archive)
        np.testing.assert_almost_equal(
            output, [[1.8501, 0.0392], [-0.2174, 0.1585], [0.1266, -0.1114], [-1.038, 2.0095]],
            decimal=4
        )
...
class TestBehaviourFunc(unittest.TestCase):
    def test_behaviour_func(self):
        ...
        np.testing.assert_almost_equal(output, [[2., 0.], [1., 1.], [0., 2.], [1., -1.]])
